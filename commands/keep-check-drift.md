...
CI (`.github/workflows/keep-drift.yml`):

```yaml
- run: python3 skills/KEEP/scripts/check_drift.py --changed
```

Local pre-commit (`.git/hooks/pre-commit`):
...
If exit 1, the PR/commit is blocked until the spec is updated or the code change reverted.
